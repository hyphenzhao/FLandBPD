...
public class bone_length_v3 extends PApplet {
...
class ROI{
  ...
  public void calculateCurvture() {
    ...
    for(int i = 0; i < ex.size(); i++) {
      ...
      float dis = (abs((y2 - y1) * x0 +(x1 - x2) * y0 + ((x2 * y1) -(x1 * y2)))) / (sqrt(pow(y2 - y1, 2.0f) + pow(x1 - x2, 2.0f)));
      if(dis > maxDis) {
        maxDis = dis;
        p3.x = ex.get(i);
        p3.y = ey.get(i);
  ...
  public void checkShape() {
    if(curvture > 0.1f) shape = "circle";
    else shape = "line";
  }
}

public float eulerDistance(float x1, float y1, float x2, float y2) {
...
public PImage getErosion(PImage oImg) {
...
public PImage getDilation(PImage oImg) { 
  PImage rImg = oImg.copy();
...
public PImage getDifferenceImage(PImage img1, PImage img2) {
...
public void sortROIs(ArrayList<ROI> input) {
...
public void loadConfigureFiles() {
...
public PImage convertToBinaryImage(PImage oImg, float threshold) {
  PImage rImg= oImg.copy();
  float maxV = 0;
...
public float getPixelValue(int o) {
  float r = (float) red(o);
...
public ArrayList<ROI> detectROI(PImage bImg) {
...
public boolean checkRange(int x, int y, int lx, int ly) {
  if(x >= 0 && x < lx && y >=0 && y < ly) return true;
  return false;
}

public float getRealLength(float lengths) {
  return lengths * depth / epixels; 
}
  public void settings() {  fullScreen(); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "bone_length_v3" };
    if (passedArgs != null) {
